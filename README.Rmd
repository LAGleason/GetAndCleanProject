...
This is an R Markdown document. This function is designed to read in a data set from the motion sensor in the Galaxy phones. In this data set 30 subjects wore the phones while doing daily activities which are listed in the activityLabels table. This analysis selects out only the data fields that evaluate mean and standard deviation. This includes the three data fields with meanFrequencies. Most data fields have an X, Y, and Z vector but not all.

The code steps include:
- reading of all files train and test
- conversion of the y_train and y_test data into names of activities vice numbers
- the use of the features table to label the column names in the x_data files
- the titling of the y_data to Activity and subject table to Subject
- after manually reviewing the features for mean and standard deviation fields these are then extracted from the x-data files
- the activity, subject, mean and standard deviation columns are all combined into one table
- the test and train data are then appended together
- the average of each data column are combined over the activity and subject such that each of the 6 activities and 30 subjects have an average for 180 rows of data across 84 variables
- the data is then written to a txt document entitled "runAnalysisByActivitySubject.txt"

This data is tidy in that each column has a single variable and each row contains one assessment of the motion sensor. The output table is also tidy in that each row contains the average of the various motion sensors grouped by activity and subject.
